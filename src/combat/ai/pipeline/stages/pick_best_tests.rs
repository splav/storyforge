...
fn run_pick(scores: Vec<f32>) -> ScoredPool {
    ...
    let mut pool = PoolBuilder::new(plans).scores(&scores).build();
...
fn pick_best_selects_highest_score() {
    ...
    assert!(
        pool.annotations[1].chosen,
        "highest-scored plan should be chosen"
    );
    assert!(
        pool.annotations[1].pick.is_some(),
        "chosen plan should have PickInfo"
    );
}
...
fn pick_jitter_skips_masked_plans() {
    ...
    assert_eq!(
        post_scores[1],
        f32::NEG_INFINITY,
        "masked plan score must be unchanged"
    );
...
fn pick_jitter_is_plan_order_invariant() {
    ...
    // Order AB.
    let (noise_ab, _) = run_jitter(
        vec![plan_a.clone(), plan_b.clone()],
        scores.clone(),
        &difficulty,
    );
    // Order BA.
    let (noise_ba, _) = run_jitter(
        vec![plan_b.clone(), plan_a.clone()],
        scores.clone(),
        &difficulty,
    );
...
fn pick_jitter_records_noise_applied_in_pick_info() {
    ...
    assert_ne!(
        pi.noise_applied, 0.0,
        "noise_applied must be non-zero under easy difficulty"
    );
}
...
fn composition_collapses_to_base_when_considerations_uniform() {
    ...
    let pool = run_pick_with_agenda(vec![initial], vec![initial], per_items, &agenda);
...
fn multi_item_pick_attributes_to_winning_item() {
    ...
    let pool = run_pick_with_agenda(vec![0.5], vec![0.5], per_items, &agenda);
    assert!(pool.annotations[0].chosen, "sole plan should be chosen");
    assert_eq!(
...
fn empty_agenda_falls_back_to_legacy_pipeline() {
    ...
    assert!(
        pool.annotations[1].chosen,
        "highest-score plan should win in legacy path"
    );
...
fn item_score_does_not_scale_with_unrelated_base_score() {
    ...
    assert!(
        ...
        "score diff must equal initial diff (1.8), got {}",
        score_b - score_a
    );
}
...
fn ineligible_item_is_skipped_in_argmax() {
    ...
    let per_items = vec![vec![
        PerItemEval {
            ...
            eligible: false,
            reject_reason: None,
            considerations: zero_considerations(),
        },
        PerItemEval {
            intent_factor: 0.0,
            tempo_factor: 0.0,
...
fn fallback_plan_can_beat_attributed_plan_with_low_cdot_when_initial_dominates() {
    ...
    let pool = run_pick_with_agenda(vec![2.0, 1.0], vec![2.0, 1.0], per_items, &agenda);
    assert!(
        pool.annotations[0].chosen,
...
fn pipeline_pick_runs_jitter_before_argmax() {
    ...
    assert_ne!(
        pi.noise_applied, 0.0,
        "noise_applied must reflect jitter contribution"
    );
}
...
mod commit_plan_tests {
    ...
    fn commit_solo_move_consumes_one() {
        let plan = plan_from(vec![PlanStep::Move {
            path: vec![hex_from_offset(1, 0)],
        }]);
        let (decision, consumed) = commit_plan(&plan, hex_from_offset(0, 0));
        assert!(matches!(
