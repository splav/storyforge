...
use crate::combat::ai::test_helpers::{
    empty_content, empty_maps, make_scoring_ctx, make_test_ctx, snapshot_from, StageTestHarness,
    UnitBuilder,
};
...
fn overlay_urgency_and_role_affinity_preserved_from_item() {
    ...
    assert!(
        ...
        "urgency should be preserved from item (0.7), got {}",
        c.urgency
    );
    assert!(
        (c.role_affinity - 0.6).abs() < 1e-5,
        "role_affinity should be preserved from item (0.6), got {}",
        c.role_affinity
    );
}
...
fn plan_aware_overlay_changes_feasibility_axis() {
    ...
    let feasibility_high = pool_high.annotations[0].per_item[0]
        .considerations
        .feasibility;
    assert!(
    ...
    let feasibility_zero = pool_zero.annotations[0].per_item[0]
        .considerations
        .feasibility;
    assert!(
...
fn feasibility_continuous_distinguishes_two_adjusted_scores() {
    ...
    let f_low = pool_low.annotations[0].per_item[0]
        .considerations
        .feasibility;
    ...
    ann_high.per_item = vec![PerItemEval::default()];
    let pool_high = run_overlay(vec![TurnPlan::default()], vec![ann_high], &agenda);
    let f_high = pool_high.annotations[0].per_item[0]
        .considerations
        .feasibility;
