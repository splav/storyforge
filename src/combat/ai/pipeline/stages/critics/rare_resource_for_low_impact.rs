...
impl PlanCritic for RareResourceForLowImpact {
    ...
    fn evaluate(&self, plan: &TurnPlan, ctx: &ScoringCtx) -> Option<CriticHit> {
        for (step_idx, step) in plan.steps.iter().enumerate() {
            ...
            let multiplier = (1.0 - 0.4 * shortfall / IMPACT_RATIO_THRESHOLD).max(MULTIPLIER_FLOOR);
...
mod tests {
    ...
    use crate::combat::ai::test_helpers::{
        assert_critic_passes, run_critic, CriticScenarioBuilder, UnitBuilder,
    };
    ...
    fn rare_resource_fires_on_canonical_case() {
        ...
        assert!(
            hit.multiplier >= MULTIPLIER_FLOOR,
            "multiplier must not go below floor {MULTIPLIER_FLOOR}"
        );
        let CriticReason::RareResourceForLowImpact {
            cost, impact_ratio, ..
        } = hit.reason
        else {
            panic!(
                "expected RareResourceForLowImpact reason, got {:?}",
                hit.reason
            );
        };
        assert_eq!(cost, 40);
        assert!(
            impact_ratio < IMPACT_RATIO_THRESHOLD,
            "impact_ratio must be below threshold"
        );
    }
    ...
    fn rare_resource_mana_cost_boundary() {
        ...
        let scn = CriticScenarioBuilder::new(caster)
            ...
            .with_ability(
                "below_threshold",
                expensive_spell("below_threshold", 29, dice),
            )
