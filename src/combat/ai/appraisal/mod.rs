...
pub(crate) mod tests {
    ...
    pub fn minimal_ability_def_with_override(
        tags: &[&str],
    ...
    fn compute_need_signals_setup_aoe_remains_zero_after_9b() {
        ...
        assert_eq!(
            signals.setup_aoe, 0.0,
            "setup_aoe must remain 0.0 — no Setup mechanic in shape"
        );
        assert!(
            signals.self_preserve < 0.05,
            "self_preserve near 0 at full HP"
        );
        assert!(
            signals.conserve_resource < 0.1,
