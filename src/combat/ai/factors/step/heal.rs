//! `StepFactor::Heal` — heal value for a Cast step.
...
pub fn compute(
    ...
    compute_offensive_for_step(ctx, step, outcome).heal
}
...
mod tests {
    ...
    fn step_factor_compute_pure_for_known_outcome() {
        ...
        let outcome = ActionOutcomeEstimate { hp_restored: 0.0, ..Default::default() };
        let needs = NeedSignals::default();
        assert_eq!(compute(&ctx, &step, &outcome, &needs), 0.0, "move step yields zero heal");
    }
}
