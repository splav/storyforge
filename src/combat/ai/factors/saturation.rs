//! Buff-saturation penalty: -0.4 per buff_class already present on the target.
//!
//! Called per Cast step inside `compute_plan_factors_sans_intent`. The caller
//! passes `pre_snap` which already reflects statuses applied by prior steps in
...
pub fn buff_saturation_penalty(
    ability: &AbilityId,
    target: Entity,
    caster: Entity,
