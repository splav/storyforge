...
/// Log schema version. Strict: analyzers must match exactly (see module doc).
/// Per-version migration history lives in git; `MIN_SUPPORTED` (below) bounds
/// what the replay tool will still read. Additive fields use `#[serde(default)]`
/// for backward read within a supported range; field reshapes are clean breaks.
