...
pub fn pick_action(
        ...
        .zip(initial_scored.into_iter().zip(initial_raw.into_iter()))
    {
        ...
        ann.factors = raw;
        // `terminal` lives on `TurnPlan.annotation` (generator side); pipeline stages
        // access it via `pool.plan_terminal(idx)`. No copy needed here.
    }
...
mod tests {
    ...
    ) -> Vec<crate::combat::ai::outcome::PipelineAnnotation> {
