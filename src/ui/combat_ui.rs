...
pub fn setup_hud(
    ...
    let txt = |size: f32| -> (TextFont, TextColor) {
        (TextFont { font: font.clone(), font_size: size, ..default() }, TextColor(Color::WHITE))
    };
    ...
    commands
        ...
        .with_children(|root| {
            ...
            root.spawn((
                ...
                Text::new(""),
                tf, tc,
                Node { flex_grow: 1.0, align_self: AlignSelf::FlexStart, ..default() },
            ));
            ...
            // ── Combat log ───────────────────────────────────────────────
            let (tf, _) = txt(13.0);
            root.spawn((HudLog, Text::new(""), tf, TextColor(Color::srgb(0.6, 0.6, 0.6))));
        });
}
...
pub fn update_phase_hint(
    ...
    let Ok(mut t) = phase_q.single_mut() else { return };
    t.0 = match phase.get() {
        CombatPhase::AwaitCommand => {
            let actor_name = ctx.active
...
pub fn update_combatants(
    ...
    db: Res<GameDb>,
    combatants: Query<
        (Entity, &Name, &Vital, &CombatStats, &Faction, &ActionPoints, &StatusEffects,
         Option<&EquippedWeapon>, Has<Dead>),
        With<Combatant>,
    >,
    mut text_q: Query<&mut Text, With<HudCombatants>>,
) {
    let Ok(mut t) = text_q.single_mut() else { return };

    let (mut players, mut enemies): (Vec<_>, Vec<_>) =
        combatants.iter().partition(|(_, _, _, _, f, _, _, _, _)| f.0 == Team::Player);
    players.sort_by_key(|(e, ..)| *e);
    enemies.sort_by_key(|(e, ..)| *e);

    let mut s = String::new();
    s.push_str("── ОТРЯД ────────────────────────────────\n");
    for row in &players { s.push_str(&fmt_row(row, &ctx, &sel, &db)); }
    s.push_str("\n── ВРАГИ ────────────────────────────────\n");
    for row in &enemies { s.push_str(&fmt_row(row, &ctx, &sel, &db)); }
    t.0 = s;
}

type Row<'a> = (
    Entity, &'a Name, &'a Vital, &'a CombatStats,
    &'a Faction, &'a ActionPoints, &'a StatusEffects, Option<&'a EquippedWeapon>, bool,
);

fn fmt_row(row: &Row, ctx: &CombatContext, sel: &SelectionState, db: &GameDb) -> String {
    let (entity, name, vital, stats, _, ap, statuses, weapon, is_dead) = row;
    ...
    let weapon_str = weapon
        ...
        .map(|wd| {
            let d = &wd.dice;
            format!(" [{}  {}d{}+{}]", wd.name, d.count, d.sides, stats.damage)
        })
        .unwrap_or_default();
...
pub fn update_ability_panel(
    ...
    let abilities: Vec<_> = ctx.active
        .and_then(|e| combatants.get(e).ok())
        .filter(|(f, _)| f.0 == Team::Player)
        .map(|(_, a)| a.0.clone())
        .unwrap_or_default();
    ...
    for (slot, mut node, mut border, mut bg, mut vis) in &mut slots {
        ...
        *border = BorderColor::all(if selected { CLR_SLOT_SEL_BORDER } else { CLR_SLOT_BORDER });
        *bg = BackgroundColor(if selected { CLR_SLOT_SEL_BG } else { CLR_SLOT_BG });
    }
    ...
    for (label, mut text, mut color) in &mut labels {
        ...
        if let Some(id) = abilities.get(idx).cloned() {
            if let Some(def) = db.abilities.get(&id) {
                ...
                *color = TextColor(if selected { Color::srgb(1.0, 0.95, 0.5) } else { Color::WHITE });
            }
        }
    }
}
...
pub fn update_turn_order(
    ...
    let Ok(mut t) = text_q.single_mut() else { return };
    if queue.order.is_empty() { t.0.clear(); return; }
    ...
    for i in 0..len {
        ...
        let Ok((name, vital, init, faction, is_dead)) = combatants.get(entity) else { continue };
        ...
        s.push_str(&format!("{marker}{team_icon}{name} {hp_str} (ini:{init_val}){dead_mark}",
            init_val = init.0));
