...
**Дефолт — комментария нет.** Литмус-тест перед каждым: «что он добавляет к
сигнатуре / имени / типам?» Если «ничего» — удалить.

Писать только:
- **Почему** выбран неочевидный подход (мотив, trade-off, ссылка на решение).
- **Невидимый из кода инвариант / контракт / подводный камень.**

Не писать никогда:
- Пересказ сигнатуры/имени/типа (`fn evaluate(..) -> Option<Hit>` не нуждается в
  «оценивает план, Some если сработал»).
- Что делает соседняя строка, когда это читается из самого кода.
- Историю разработки (фазы, номера шагов, «было/стало») — она в git.
...
cargo nextest run --features dev                            # только storyforge pkg — МЕНЬШЕ тестов, см. note ниже
...
- **Полный прогон:** `cargo nextest run --workspace --features dev`.
