...
Каждый `ActionCandidate` оценивается по 9 факторам, нормализованным в 0..1 внутри батча. Финальный скор = `dot(factors, role_weights) + noise`.
...
| `scarcity` | `swing_value - resource_ratio` (см. ниже) | `/ max в батче` |
