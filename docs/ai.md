...
| `factors/` | 10-факторный скоринг: `mod.rs` (`ScoredStep`, `compute_factors`), `offensive.rs` (damage/heal/kill_now/kill_promised/cc + `aoe_area`), `scarcity.rs`, `adjustments.rs` (reservations + `crit_fail_adjusted`), `tempo.rs` (plan-terminal `tempo_gain`), `saturation.rs` (buff-redundancy penalty), `survival.rs` (plan-level `self_survival`) |
| `intent.rs` | `TacticalIntent` — выбор стратегической цели + `intent_score` по `ScoredStep` + `AiMemory` (stickiness + `last_plan` для plan freeze) + `PlanSnapshot` / `StoredPlan` (инвалидация) |
...
`intent_score(intent, step, &ScoringCtx) -> f32` вычисляет alignment одного шага плана.

**`FocusTarget` и `ApplyCC`** используют dot-product факторов × intent-специфичный вектор весов (`IntentWeights`). Вначале `compute_factors(step_ctx, step)` вычисляет 13 факторов; затем `filter_offensive_for_target` обнуляет offensive-оси (damage/kill_now/kill_promised/cc) для шагов, не направленных на интент-цель:
...
| `src/combat/ai/factors/offensive.rs` | damage/heal/kill_now/kill_promised/cc выход если эффект туда попадает |
