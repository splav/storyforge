...
MVP1 — **архитектурный refactor**. Он убирает lethal-AoO hard-mask и перестаёт душить self-lethal планы в `-∞` — они возвращаются в pool и становятся сравнимыми. Но **экономика размена** (выгодно ли умереть ради убийства конкретной цели) решается только когда в measurement появится симметричный сигнал `expected_self_loss` — отдельный factor, запланированный на MVP2.
