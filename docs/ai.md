...
3. pick_action:
   ...
   e. sanity_adjust_plans: мультипликативные штрафы (survival quadratic,
      LoS blindspot, retreat trap, self-AoE, AoO risk).
   f. Если intent == ProtectSelf: маскировать не-defensive в -∞;
      если defensive нет — rescore под LastStand.
   g. pick_best_plan: mercy окно → rerank по cruelty → top-K sampling
...
После scoring, если intent == ProtectSelf: не-defensive планы → `−∞`. Defensive = SingleAlly/Myself cast ИЛИ Move на безопасную клетку ИЛИ cast с клетки safer-by-`defensive_tile_margin`. Если defensive нет — rescore под `LastStand`.
