...
| `factors/` | 10-факторный скоринг: `mod.rs` (`ScoredStep`, `compute_factors(ctx, step, outcome)`), `offensive.rs` (читает `ActionOutcomeEstimate` facts + применяет `policy::*` — damage/heal/cc/kill_now/kill_promised), `scarcity.rs`, `adjustments.rs` (reservations + `crit_fail_adjusted`), `tempo.rs` (plan-terminal `tempo_gain`), `saturation.rs` (buff-redundancy penalty), `survival.rs` (plan-level `self_survival`) |
| `intent.rs` | `TacticalIntent` — выбор стратегической цели + `intent_score(intent, step, step_ctx, outcome)` + `AiMemory` (stickiness + `last_goal: Option<StoredGoalContext>` для goal-preserving repair) |
| `outcome/` | `ActionOutcomeEstimate` (17 fact-полей: enemy_damage, ally_damage, self_damage, p_kill_now, p_kill_soon, cc_turns_applied, vulnerability_applied, armor_shred_applied, hp_restored, path_max_danger, mp_spent, ap_spent, mana_spent, rage_spent, other_resource_spent + per-entity breakdowns) + `PlanAnnotation`. Builder — `outcome::builder::{from_sim_step, hypothetical}`. Policy module `combat::ai::policy::*` содержит named pure functions; `compute_score_core` и `score_action` удалены в step 4.5/4.12. |
| `scoring.rs` | HP-эквивалент helpers: `estimate_st_damage`, `estimate_damage_horizon`, `status_applications`, `stun_denial_value`, `applies_cc`. Центральный `score_action` удалён в step 4.5; `compute_score_core` удалён в step 4.12 — формулы переехали в `policy::*`. |
...
**Policy module.** `combat::ai::policy::` (5 sub-modules: `damage`, `heal`, `friendly_fire`, `status`, `cc`) — pure functions `fn(facts, context) -> f32`. Применяются в `compute_offensive` и `future_value::λ_attack`. `compute_score_core` и `score_action` удалены в step 4.12.
...
## Policy формулы (combat::ai::policy/)

HP-эквивалентные оценки fact-полей. Живут в `src/combat/ai/policy/` как named pure functions — единственный source of truth для «как мы оцениваем этот факт». `compute_score_core` и `score_action` удалены в step 4.12; вместо них — policy sub-modules.
...
*Шаг 11 (schema v32). Источники: `intent/bands.rs`, `intent/agenda.rs`, `intent/considerations.rs`, `pipeline/stages/pick_best.rs`.*
...
где:
- `score_initial` — score после initial `score_plans_with_raw` pass (до pipeline stages)
...
| `src/combat/ai/policy/` | HP-эквивалент formulas (`damage::value`, `heal::value`, `cc::value`, `friendly_fire::penalty`) — если новый эффект нужно скорить, добавь named pure function |
...
Добавление новой оси в outcome vector — для future consumer'ов (critics, step 10; geometry, step 17).
...
| Этот документ | Строка в таблице «Outcome vector (outcome/)» |
