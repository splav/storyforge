...
**Результат (post-playtest, 26 апр. 2026):**

Mining на 6 v26-логах post-6.7 (27 plan_divergence events):

| Метрика | Pre-6.7 | Post-6.7 | Gate | Статус |
|---|---|---|---|---|
| `goal_preserved` (combined) | 54.1% | **63.0%** | ≥60% | ✓ |
| `method_delivered` | 5.4% | **22.2%** | ≥10% | ✓ |
| `voluntary` | 13.5% | **22.2%** | ≤10% | ✗ (см. 6.8) |
| `reactive` | 32.4% | 14.8% | by design | — |
| `invalidating` | 0% | 0% | 5–10% | ✗ (см. 6.9) |
| `ttl_expired` | 0% | 0% | 2–5% | ✗ (см. 6.9) |
| Cross-round entries | 0 | ~10/27 | новое | ✓ работает |

Cross-round preservation работает (видно method_delivered с `ttl=0.5` в логах). Voluntary вырос с 13.5% до 22.2% — не регрессия, а **demarcation**: pre-6.7 cross-round transitions были невидимы. TTL/Invalidating всё ещё 0% — scenario coverage gap, не баг lifecycle.

Golden rebaseline'ed: `logs/golden_post_step6.jsonl` (99 records, round-trip 0/99).
...
### 6.9. Scenario coverage — `continuation_*` ai_scenarios

**Проблема.** TTL/Invalidating всё ещё 0% в mining'е post-6.7. Это **не баг** 6.7 lifecycle, а scenario coverage gap:
- TTL=2 не достигается потому что бои в текущем corpus короткие (45/48 actor-turns в раундах 1–2).
- Invalidating cross-round (target_dead между ходами actor'а) не фиксируется потому что в bell_under_veil/demo_campaign maps targets не умирают между ходами committed actor'а.

Также 5 `continuation_*` ai_scenarios из §6.6b (target death, cosmetic, relevant-with-preservation, setup-multi-tick, ttl-expiration) остаются pending с момента 6.6b — corpus теперь есть.

**Scope.**

Создать 5 fixtures в `tests/ai_scenarios/snapshots/`:

1. **`continuation_target_dies_replan`** — overlay ассертит, что после смерти target'а между ходами актор переключается на следующий, log пишет `goal_abandoned_invalidating`.
2. **`continuation_cosmetic_rage_tick_no_replan`** — overlay ассертит `goal_preserved_method_delivered` после AoO-induced rage tick.
3. **`continuation_actor_hp_drop_relevant`** — Relevant mismatch, но goal preserved.
4. **`continuation_setup_aoe_two_ticks`** — multi-tick SetupAOE, planned_ability preserved через 2 хода.
5. **`continuation_ttl_expires`** — overlay на 3-й tick'е после store (TTL=2): outcome = `goal_abandoned_ttl_expired`.

**Зафиксированные решения по развилкам.**

**1. Fixtures — production-extracted, не handwritten** (1a). Из реальных playtest'ов выбираем подходящие моменты, экспортируем в `.jsonl` snapshot + `.expected.toml` overlay.

**Альтернатива (1b):** генерировать fixtures программно. Отвергнута: handwritten fixtures хрупки к minor scoring drift; production-extracted — стабильны.

**2. Подходящие сценарии для fixtures.**
- `target_dies_replan` — beastblood_raid (быстрые kills).
- `cosmetic_rage_tick` — любой melee actor с rage resource (Грозорождённый Буревестник).
- `actor_hp_drop_relevant` — Tank в bell_crypt получает урон от AoE.
- `setup_aoe_two_ticks` — Control class (нет в текущих corpus'ах? нужен playtest с Control юнитом).
- `ttl_expires` — длинный бой 3+ раунда; нужен playtest до конца боя.

**Scope.**

1. **Playtest extension.** Возможно нужен дополнительный playtest на длинной встрече (3+ rounds для TTL fixture; SetupAOE control class для setup fixture).
2. Создание 5 `.jsonl` + 5 `.expected.toml` в `tests/ai_scenarios/snapshots/continuation_*`.
3. `cargo test --test ai_scenarios` зелёный (9 + 5 = 14 scenarios).
4. Mining на v26-логах из новых fixtures: `ttl_expired > 0%` и `invalidating > 0%`.

**Gate.**
- `cargo test --test ai_scenarios` все 14 зелёные.
- Mining на новых v26-логах (включая длинный бой): `ttl_expired ≥ 1`, `invalidating ≥ 1`.

**Эстимейт:** 1.0 день (5 fixtures × ~1.5 часа извлечения + опциональный playtest на длинной встрече).
