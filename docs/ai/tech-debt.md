...
Phase 2 ✅ ── R8-lite / Score Effect Engine ──────────────────────────────
       ...
       Status: ✅ closed (commits a14fb2f..Step7/d1062b6+). 908 passed, 0 diverged.
...
Phase 3 ✅ ── A2 KillableGate semantic fix ────────────────────────────────
       ...
       Done: A2 closed, f32_finite removed. ann.score always finite.

Phase 4 ✅ ── B2 schema observability cleanup ────────────────────────────
       Achieved via TLE-1..TLE-3a: score_trace enriched with diagnostic
       detail (SanityRule, CriticKind+CriticReason, mask original_score),
       mining/replay migrated to read enriched trace, legacy fields
       (modifiers/sanity/critics/contract) removed from PlanAnnotation.
       No schema bump — old logs deserialize via serde ignore-unknown.
       ann.adaptation retained (input-state, not score-effect observability).
...
Phase 5 ── Opportunistic ─────────────────────────────────────────────────
       ...
       3-bucket SelectionKey policy slice (gated > masked fallback)
       TLE-3b sum-type MultiplierHit cutover (replaces Option<Detail>
         with type-enforced enum; requires fixture regeneration via
         gameplay or custom backward-compat deserializer; skipped for now —
         Option<Detail> + debug_assert is acceptable substitute)
...
| `f32_finite` workaround, `debug_assert_eq` invariants, `score_initial` в annotation, pre_scores buffer dance | Phase 2 (centralized) — `score_initial` и `pre_scores` dance закрыты; `debug_assert_eq` NEG_INFINITY ✅ Phase 2 (Step 6); `f32_finite` ✅ Phase 3 (Step 5) |
| A2 + SelectionKey | ✅ Phase 3 |
| B2 schema cleanup | ✅ TLE (Phase 4 closed) |
| B3, C3, C5, B1-rest | Phase 5 (opportunistic) |
| 3-bucket SelectionKey (gated > masked) | Phase 5 policy slice |
| TLE-3b sum-type MultiplierHit | Phase 5 (opportunistic, when fixtures regen pipeline available) |
...
**Status:** ✅ closed (`ai/Phase3`, Steps 1–5). KillableGate emits Gate-only via Score Effect Engine; SelectionKey { selectable, score } replaces NEG_INFINITY hidden channel. ann.score always finite. f32_finite serde adapter removed (Phase3-Step5).
...
- `f32_finite` serde adapter в `outcome/mod.rs` — **Status: ✅ closed (`ai/Phase3-Step5`)**. Adapter удалён из `src/combat/ai/log/serde_helpers.rs` и аннотация `#[serde(with = ...)]` с поля `score` снята. ann.score финитен → стандартный serde.
- `debug_assert_eq!(ann.score_trace.compute(), f32::NEG_INFINITY)` в `killable_gate/mod.rs` — **Status: ✅ closed (`ai/Phase2-Step6`, removed during KillableGate engine migration)**.
...
**Status B2-writer-dedup:** ✅ closed (`ai/Phase2`). Drive-loop (`pipeline/effects.rs::apply_score_effect_stage`) — sole writer обоих каналов: `score_trace` и legacy fields (`modifiers`, `sanity`, `critics`, `contract`). Стадии score-effect family возвращают `Vec<AppliedEffect>`; drift между каналами невозможен по конструкции.

**Status B2-schema-cleanup:** ✅ closed via TLE (commits 10bc30b → 14f1c4b).
- TLE-1: `MultiplierHit.detail`, `MaskHit.original_score` enriched runtime types; drive-loop derives from paired observability; `score_trace_log` mirror carries the detail (additive serde).
- TLE-2: mining (`mine_ai_logs`) и replay verbose читают enriched trace primary; legacy fallback paths добавлены для v33/v34 backward-compat; shadow-compare invariant ловил drift во время coexistence.
- TLE-3a: legacy fields (`modifiers`, `sanity`, `critics`, `contract`) удалены из `PlanAnnotation`; getters/builders/mirror-test удалены; mining fallback paths удалены. **No schema bump** — старые JSONL логи deserialize через serde ignore-unknown.

`ann.adaptation` retained — это input-state, не score-effect observability. Phase 4 не покрывает.

TLE-3b (sum-type cutover для `MultiplierHit`, чтобы убрать `Option<Detail>` runtime check) — **deferred to Phase 5 opportunistic**: требует либо fixture regeneration (manual gameplay), либо permanent custom Deserialize backward-compat layer. Текущий debug_assert в `apply_effect` обеспечивает практическую type-safety для new captures.
...
**Status B1-soft:** ✅ closed (`ai/Phase2`). Поля `score`, `score_trace`, `modifiers`, `sanity`, `critics`, `contract` переведены в `pub(crate)`. Геттеры (`score()`, `modifiers()`, `sanity()`, `critics()`, `contract()`, `score_trace()`) и builder'ы (`with_score()`, `with_modifiers()`, …) — публичный API для external consumers. `set_score()` + `apply_effect()` + `recompute_score_from_trace()` — intra-crate writer API. Drive-loop — sole writer score/effect channels.

**Status B1-rest:** Phase 5 (opportunistic). Остальные ~24 поля остаются `pub` до явных регрессий.
...
**Status:** ✅ closed. Commits a14fb2f..d1062b6 (Steps 1–7). 908 тестов passed, golden_smoke 0/114 diverged. Score/effect architecture централизована; B1-soft и B2-writer-dedup закрыты.
