...
| `f32_finite` workaround, `debug_assert_eq` invariants, `score_initial` в annotation, pre_scores buffer dance | Phase 2 (centralized) — `score_initial` и `pre_scores` dance закрыты; `f32_finite` и `debug_assert_eq` NEG_INFINITY остаются до Phase 3 (A2 KillableGate) |
