...
Phase 4 ── B2 schema observability cleanup [DEFERRED] ────────────────────
       Reason: score_trace_log lacks diagnostic detail carried by legacy
       fields (SanityRule, CriticKind+CriticReason, ContractMaskHit
       .original_score, AdaptationData). Removing legacy fields without
       enrichment would reduce mining/replay quality.
       Writer-drift risk already closed in Phase 2 (drive-loop sole writer).
       Blocked by: TLE (TraceLog enrichment) — see section below.
...
TLE   ── TraceLog enrichment (unblocks Phase 4) ──────────────────────────
       Goal: make score_trace_log diagnostic-complete enough to replace
       legacy fields entirely. Required additions:
         - sanity multiplier hits carry SanityRule
         - critic multiplier hits carry CriticKind + CriticReason
         - mask hits carry original_score
         - adaptation surfaced (AdaptationReason + original_score)
       Once enriched, mining/replay migrate, then schema v35 drops legacy
       fields. ~3-4 days when prioritised.
...
| B2 schema cleanup | [DEFERRED — needs TLE enrichment] |
...
**Status B2-schema-cleanup:** **[DEFERRED]**. Audit показал: `score_trace_log` теряет диагностику против legacy fields (SanityRule, CriticKind+CriticReason, ContractMaskHit.original_score, AdaptationData). Прямой drop ухудшит mining/replay quality. Сначала нужен **TLE (TraceLog enrichment)** — обогатить score_trace_log до полноты, потом mining/replay → trace, потом schema bump v35 → drop legacy. Writer-drift risk уже закрыт в Phase 2; legacy fields живут как **derived mirrors**, не independent writers. Не приоритет, пока не появится конкретная боль (размер JSONL / mining performance).
