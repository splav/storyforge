...
| **5c.2** ✓ | TomlContentView (much smaller after 5c.1) | **SHIPPED.** New `crates/combat_engine/src/toml_content_view.rs`: `TomlContentView` with `load_from_dir(data_dir)` + `empty()`. Implements 4-method `ContentView` trait. Duplicates TOML record structs (Path A — bridge parsers are Bevy-tied). Parity test `tests/toml_content_view_parity.rs` checks 18 abilities + 10 statuses vs bridge view. Also fixed pre-existing bugs from 5c.1: `make_unit` missing `aoo_dice` field in state.rs tests; `rng_count.rs` test setting `caster_context.weapon_dice` instead of `aoo_dice`. |
