...
`Default` использует глобальный `TacticalIntent` для скоринга intent-column. `LastStand` переиспользует `evaluate_last_stand_step` — оценивает «последнее полезное действие». `Flee` переиспользует `evaluate_flee_step` — юнит максимизирует дистанцию до ближайшего врага (Move оценивается дельтой расстояния), self-heal/self-buff разрешены (`+0.3`). И `LastStand`, и `Flee` обходят глобальный intent целиком в `intent_score` (ранний return).

**Важно: offensive-касты под `Flee` отбрасываются на этапе генерации** (`enumerate_next_steps` в `plan/generator.rs` пропускает абилки с `target_type ∈ {SingleEnemy, Ground}`, когда `actor.forced_mode() == Flee`), а не просто штрафуются в скоринге. Причина: offensive damage/tempo — это отдельные step-факторы, которые скорятся независимо от intent-колонки; одного отрицательного intent-скора (`-1.0` в `evaluate_flee_step`, оставлен как defense-in-depth) недостаточно — атака могла бы победить на «сыром» уроне. В отличие от `LastStand`, чья цель совпадает с offensive-факторами, цель `Flee` им противоположна, поэтому нужен hard-drop кандидатов (ТЗ §9: «Cast-кандидаты выкидываются»).
