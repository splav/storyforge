...
8. **Не валит ли его critic?** — `CriticsStage::first_wave` применяет 6 multipliers; `score_trace_log.multipliers` (filter kind=Critic) в JSONL покажет hits с `MultiplierDetail::Critic { critic, reason }`. Legacy `ann.critics` field удалён в TLE-3a.
