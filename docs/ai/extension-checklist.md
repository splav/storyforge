...
| Файл | Что |
|---|---|
| `src/combat/ai/critics/<name>.rs` | Реализация `PlanCritic` trait |
| `src/combat/ai/pipeline/stages/critics.rs::CriticsStage::first_wave` | Регистрация в композиции |
...
| `src/combat/ai/modifiers/trade_bonus.rs` | Уже читает через public helper — при изменении формулы больше ничего |
