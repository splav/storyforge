...
**Общий принцип после Phase 5/6**: правишь движок (`crates/combat_engine/`) — он канонический → правишь content (TOML парсер) → правишь мост (`src/combat/bridge/` — translator/projection, если новое поведение должно видеть UI) → правишь AI (scoring → intent → outcome estimate). Sim теперь = вызов `engine_step()`; отдельной реализации `apply_primary` больше нет.

Engine — это `crates/combat_engine/` (Bevy-free), bridge — модуль `src/combat/bridge/`. ECS-проекция (`Vital`, `ActionPoints`, `Reactions`, и т.п.) пишется ТОЛЬКО через `project_state_to_ecs`; см. `tests/projection_isolation.rs` (Phase 6 D6 guard).
...
Тесты: позитивный + негативный кейсы в `tests/combat_engine/legality_parity.rs` (или соседний engine-тест) + генератор-тест в `src/combat/ai/plan/generator::tests`.
...
| `src/combat/bridge/translate.rs::translate_one` (exhaustive match по `Event`) | Перевод нового `Event` в `CombatEvent` для UI / animation — только если поведение наблюдаемо |
| `src/combat/ai/outcome/builder.rs::from_sim_step` | Заполнение `ActionOutcomeEstimate` (damage / heal / cc / etc.) для нового эффекта. Сам сим менять не нужно — `SimState` гоняет engine `step()` напрямую, новый `Effect` виден автоматически |
...
Тесты: engine arm — `tests/combat_engine/effect.rs`; replay — добавить scenario в `tests/combat_engine/replay.rs` если детерминизм нетривиален.
...
| `crates/combat_engine/src/content.rs::StatusBonuses` + `effect.rs` (`RefreshAggregates`) | Численный бонус агрегируется движком в `Unit.armor_bonus`/`speed`/`damage_taken_bonus`; AI читает через `UnitView` |
...
Любое изменение, ломающее формат JSONL-логов (новое/изменённое поле в `PlanAnnotation`, `AiCache` / engine `CombatState` (через `BattleSnapshot`), `IntentBlock`, `ScoreTraceLog`, и т.п.).
...
| v44 → v46 | `EnvObject.revealed: bool` replaced by `owner: Option<Team>` + `revealed_to: TeamSet` (per-trap ownership + per-team reveal); baseline renamed `baseline_v44.jsonl` → `baseline_v46.jsonl`; `MIN_SUPPORTED` unchanged (43); no fixture recapture needed (no env objects in any fixture or baseline) | ch2 Wave 3 |
...
| `tests/combat_engine/replay.rs` | Если изменилось поведение `step()` — добавить/обновить canonical scenario |
...
| v44 → v45 | `EffectSource` enum introduced; `Effect::Damage.source`, `Effect::ApplyStatus.applier`, `DotDamageCtx.source`, `ActiveStatus.applier` widened from `UnitId` to `EffectSource { Unit \| Env }` |
| v45 → v46 | `EnvObject.revealed: bool` replaced by `owner: Option<Team>` + `revealed_to: TeamSet` (per-trap ownership + per-team reveal); new fields carry `#[serde(default)]` for graceful degradation |
...
| `src/combat/bridge/project.rs::project_state_to_ecs` | Добавь arm записи (engine state → component) |
| `src/combat/bridge/bootstrap.rs::bootstrap_combat_state` / `from_ecs` | Если bootstrap читает из ECS — добавь arm чтения |
...
**Контракт:** mutation engine-projected компонента может происходить ТОЛЬКО внутри `src/combat/bridge/` (или allowlisted spawn/init paths). Test `engine_projected_components_only_written_by_bridge` падает на любое нарушение.
