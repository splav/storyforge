...
### 1.8. `kill_max` и `focus_max` — параллельная патология с intent_max ✓ fixed (09659d5)
...
Последствие то же, что было у intent (issue 1.3, fixed): план `[Move-random, Move-random, Cast-kill-X]` получал `kill_max=1.0` и `focus_max=target_priority(X)` за буквально не-фиксирующийся Cast в шаге-2.

**Исправлено:** зеркально 1.3 — `if idx < committed` guard внутри существующей Cast-only ветки перед обновлением `kill_max` / `focus_max`. Discounted-sum факторы (damage/cc/heal/scarcity) оставлены как есть — они не flagged аудитом, их геометрический discount уже моделирует execution uncertainty.
...
| 1.8 `kill_max`/`focus_max` — параллельная 1.3 патология | корректность скоринга | средний | ✓ 09659d5 |
