...
fn parity_pure_move_no_enemies() {
    let actor_id = ent(1);
    let snap = snapshot_from_pairs(vec![make_unit(1, Team::Player, 0, 0, 20, None, 0)], 1);
    let path = vec![
        hex_from_offset(1, 0),
        hex_from_offset(2, 0),
        hex_from_offset(3, 0),
    ...
    // MP: 6 - 3 = 3.
    let engine_mp = engine_unit.pools[storyforge::combat_engine::PoolKind::Mp]
        .map(|(c, _)| c)
        .unwrap_or(0);
    let sim_mp = sim_unit.pools[storyforge::combat_engine::PoolKind::Mp]
        .map(|(c, _)| c)
        .unwrap_or(0);
    ...
    assert_eq!(
        engine_unit.hp(),
        sim_unit.hp(),
        "hp parity: no enemies → no damage"
    );
}
...
fn parity_move_no_aoo_stays_adjacent() {
    ...
    assert_eq!(
        actor_start.unsigned_distance_to(enemy_pos),
        1,
    ...
    assert_eq!(
        engine_state.unit(enemy_uid).unwrap().reactions_left,
        1,
        "engine: enemy reaction not consumed"
    );
    assert_eq!(
        sim_snap.unit(enemy_id).unwrap().reactions_left,
        1,
...
fn parity_aoo_chain_two_enemies() {
    ...
    assert_eq!(
        start.unsigned_distance_to(ea_pos),
        1,
        "enemy A adjacent to start"
    );
    assert_ne!(
        step1.unsigned_distance_to(ea_pos),
        1,
        "enemy A not adjacent after step 1"
    );
    ...
    assert_eq!(
        step2.unsigned_distance_to(eb_pos),
        1,
        "enemy B adjacent to (3,0)"
    );
    ...
    assert_eq!(
        engine_actor.pos,
        hex_from_offset(2, 0),
        "mover stopped at step1 after AoO halt"
    );

    // Only step1's DecrementMP fired; remaining 2 steps were skipped: 5 - 1 = 4.
    let engine_mp = engine_actor.pools[storyforge::combat_engine::PoolKind::Mp]
        .map(|(c, _)| c)
        .unwrap_or(0);
    assert_eq!(
        engine_mp, 4,
        "only one MP consumed (halt preserved remaining MP)"
    );

    // Enemy A fired; enemy B was never reached.
    assert_eq!(
        engine_state.unit(ea_uid).unwrap().reactions_left,
        0,
        "engine: enemy A reaction consumed"
    );
    assert_eq!(
        engine_state.unit(eb_uid).unwrap().reactions_left,
        1,
...
fn parity_aoo_kills_mover_mid_path_truncates() {
    ...
    assert_ne!(
        step1.unsigned_distance_to(eb_pos),
        1,
    ...
    assert!(
        engine_result.is_ok(),
        "engine should return Ok under truncation semantics"
    );

    let engine_actor = engine_state.unit(actor_uid).unwrap();
    assert_eq!(engine_actor.hp(), 0, "engine: mover dead after lethal AoO");
    assert_eq!(
        engine_actor.pos, step1,
        "engine: mover at step1 (hit position)"
    );
    ...
    let sim_actor = sim_snap
        .unit(actor_id)
        .expect("actor present in sim snapshot");
    assert_eq!(
        sim_actor.hp(),
        engine_actor.hp(),
    ...
    assert_eq!(
        sim_actor.pos, engine_actor.pos,
