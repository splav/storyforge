...
fn snapshot_blocked_los(from: Hex, to: Hex, blocked: &HashSet<Hex>) -> bool {
    ...
    let adapter = SnapshotActionState { content: &content, snap: &snap };
...
fn bevy_actions_is_blocked_los_matches_has_los() {
    ...
    assert_eq!(bevy_blocked_los(a, b, &empty), direct_blocked_los(a, b, &empty),
    ...
    assert_eq!(bevy_blocked_los(a, b, &blocked), direct_blocked_los(a, b, &blocked),
    ...
    assert_eq!(bevy_blocked_los(a, b, &endpoint_blocked), direct_blocked_los(a, b, &endpoint_blocked),
        "endpoint blockers only: Bevy should agree with has_los (endpoints excluded)");

    // Self-LOS — always clear.
    assert!(!bevy_blocked_los(a, a, &blocked), "self-LOS should never be blocked");
}
...
fn snapshot_actions_is_blocked_los_matches_has_los() {
    ...
    let empty: HashSet<Hex> = HashSet::new();
    assert_eq!(snapshot_blocked_los(a, b, &empty), direct_blocked_los(a, b, &empty),
        "empty: Snapshot agrees with has_los");

    let blocked: HashSet<Hex> = [mid].into_iter().collect();
    assert_eq!(snapshot_blocked_los(a, b, &blocked), direct_blocked_los(a, b, &blocked),
        "blocker on mid: Snapshot agrees with has_los");

    let endpoint_blocked: HashSet<Hex> = [a, b].into_iter().collect();
    assert_eq!(snapshot_blocked_los(a, b, &endpoint_blocked), direct_blocked_los(a, b, &endpoint_blocked),
        "endpoint blockers only: Snapshot agrees with has_los");

    assert!(!snapshot_blocked_los(a, a, &blocked), "self-LOS should never be blocked");
}
...
fn engine_action_state_is_blocked_los_matches_has_los() {
    ...
    let empty: HashSet<Hex> = HashSet::new();
    assert_eq!(engine_blocked_los(a, b, &empty), direct_blocked_los(a, b, &empty),
        "empty: Engine agrees with has_los");

    let blocked: HashSet<Hex> = [mid].into_iter().collect();
    assert_eq!(engine_blocked_los(a, b, &blocked), direct_blocked_los(a, b, &blocked),
        "blocker on mid: Engine agrees with has_los");

    let endpoint_blocked: HashSet<Hex> = [a, b].into_iter().collect();
    assert_eq!(engine_blocked_los(a, b, &endpoint_blocked), direct_blocked_los(a, b, &endpoint_blocked),
