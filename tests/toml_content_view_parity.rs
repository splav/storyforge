...
impl AbilityExt for AbilityDef {
    fn costs_eq(&self, other: &AbilityDef) -> bool {
        ...
        self.costs
            .iter()
            .zip(&other.costs)
            .all(|(a, b)| a.resource == b.resource && a.amount == b.amount)
    }
}
...
fn statuses_eq(a: &[StatusApplication], b: &[StatusApplication]) -> bool {
    ...
    a.iter().zip(b).all(|(x, y)| {
        x.status == y.status && x.duration_rounds == y.duration_rounds && x.on == y.on
    })
}
...
fn toml_content_view_matches_ecs_content_view() {
    ...
    let toml_view =
        TomlContentView::load_from_dir(data_dir).expect("TomlContentView::load_from_dir failed");
    ...
    for id in bridge_view.abilities.keys() {
        ...
        match (&expected, &got) {
            ...
            (Option::None, Some(_)) => {
                failures.push(format!(
                    "ability_def({id}): present in TomlContentView but absent in bridge"
                ));
            }
    ...
    for id in bridge_view.statuses.keys() {
        ...
        match (&expected, &got) {
            ...
            (Option::None, Some(_)) => {
                failures.push(format!(
                    "status_def({id}): present in TomlContentView but absent in bridge"
                ));
            }
    ...
    for id in bridge_view.unit_templates.keys() {
        ...
        match (&expected, &got) {
            ...
            (Option::None, Some(_)) => {
                failures.push(format!(
                    "unit_template({id}): present in TomlContentView but absent in bridge"
                ));
            }
            (Option::None, Option::None) => {}
        }
    ...
    assert!(
        n_abilities > 0,
        "no abilities loaded — parity test is vacuous"
    );
    assert!(
        n_statuses > 0,
        "no statuses loaded — parity test is vacuous"
    );
...
fn environment_ability_parity_scout_traps() {
    ...
    let engine_def = toml_view
        .ability_def(&id)
        .cloned()
        .expect("scout_traps not found in TomlContentView");
    let bridge_def = bridge_view
        .abilities
        .get(&id)
    ...
    match &engine_def.effect {
        EffectDef::RevealEnvInRange { range } => assert_eq!(
            *range, 2,
            "TomlContentView: scout_traps reveal range should be 2"
        ),
    ...
    match &bridge_engine_def.effect {
        EffectDef::RevealEnvInRange { range } => assert_eq!(
            *range, 2,
            "BridgeContentView: scout_traps reveal range should be 2"
        ),
